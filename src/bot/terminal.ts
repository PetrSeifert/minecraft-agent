...
async function runCommand(bot: MinecraftBot, agent: Agent, input: string): Promise<string | null> {
  ...
  switch (command) {
    ...
    case "inspect": {
      ...
      return formatResult(
        agent.world.inspectVisibleArea({
          maxDistance,
        }),
      );
    }
    case "findblock": {
      ...
      return formatResult(agent.world.findBlockByName(args[0], { maxDistance }));
    }
    case "block": {
      if (args.length < 3) {
    ...
    case "follow": {
      ...
      return formatResult(
        agent.pathing.followEntity(target, args[1] ? parseNumber(args[1], "range") : 2),
      );
    }
    case "stop":
      agent.pathing.stop();
    ...
    case "craft": {
      ...
      return formatResult(
        await agent.actions.craftItem(args[0], args[1] ? parseNumber(args[1], "count") : 1),
      );
    }
    case "place": {
      if (args.length < 4) {
    ...
    case "retreat": {
      ...
      return formatResult(await agent.safety.retreatFromNearestHostile(minDistance));
    }
    case "events": {
      ...
      const lines = agent.events.recent(count).map((event) => {
        ...
        return summary ? `[${event.id}] ${event.type}: ${summary}` : `[${event.id}] ${event.type}`;
      });
...
export function createTerminal(bot: MinecraftBot, agent: Agent, terminal: Interface): void {
