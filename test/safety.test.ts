...
function createSafetyBot(options: {
  blocks?: Array<ReturnType<typeof createBlock>>;
  fireFlag?: boolean;
  getPathTo?: (goal: { x: number; y: number; z: number }) => { status: 'noPath' | 'success' | 'timeout' };
  position?: Vec3;
} = {}) {
...
test('escapeDanger enters water instead of stopping beside it when extinguishing fire', async () => {
  ...
  const gotoCalls: Array<{ options?: Record<string, unknown>; position: { x: number; y: number; z: number }; range?: number }> = [];
  const safety = createSafetyModule(bot as never, {
    ...
    pathing: {
      ...
      goto(position: { x: number; y: number; z: number }, range?: number, options?: Record<string, unknown>) {
        gotoCalls.push({
          options,
          position,
          range,
