...
function createPlannerHarness(options: {
  ...
  const planner = createPlannerModule(bot as never, {
    client: {
...
test('planner leaves the goal unchanged on failure and on materially identical goals', async () => {
  ...
  assert.equal(
    (plannerEvents(sameGoalHarness.events, 'planner:success')[0]?.payload as { changed?: boolean })?.changed,
    false,
  );
...
test('planner skips overlapping replans while one request is in flight', async () => {
  ...
  const harness = createPlannerHarness({
    chooseGoal: () => new Promise<string>((resolve) => {
...
test('planner triggers on spawn, manual requests, goal clear, death, and interval', async () => {
  ...
  const triggers = plannerEvents(harness.events, 'planner:request').map((event) =>
