...
function createEntity(name: string, position: Vec3, options: {
  id?: number;
  type?: string;
  username?: string;
} = {}) {
...
test('inspectVisibleArea reports full-radius blocks on all sides independent of yaw', () => {
  ...
  const southFacing = createWorldModule(createFakeBot({
    blocks,
    yaw: 0,
  }) as never);
  const northFacing = createWorldModule(createFakeBot({
    blocks,
    yaw: Math.PI,
  }) as never);
...
test('inspectVisibleArea filters invisible blocks, deduplicates by nearest block, and reports hazards', () => {
  ...
  const bot = createWorldModule(createFakeBot({
...
test('inspectVisibleArea preserves requireSpawned behavior before spawn', () => {
  const world = createWorldModule(createFakeBot({
    position: null,
  }) as never);
